...
int nova_extension_matches(const char* path, const char* expected_ext) {
    ...
    size_t len2 = strlen(expected_ext);
...
char* nova_load_file_to_arena(struct NovaArena* arena, const char* path, size_t* out_size) {
    ...
    if (size == 0) {
        ...
        *out_size = 0;
        ...
        char* empty = nova_arena_alloc(arena, 1);
        if (empty) empty[0] = '\0';
        return empty;
    }

    char* buffer = (char*)nova_arena_alloc(arena, (size_t)size);
